...
def configure_app_secrets() -> None:
    ...
    jwt_secret = _require_secret_value(secret, JWT_SECRET_FIELD)
    settings.JWT_SECRET_KEY = jwt_secret
    security_settings.set_secret_key(jwt_secret)
    settings.set_database_password(_require_secret_value(secret, DATABASE_PASSWORD_FIELD))
    settings.MINIO_SECRET_KEY = _require_secret_value(secret, MINIO_SECRET_KEY_FIELD)
    settings.SFTP_PASSWORD = _require_secret_value(secret, SFTP_PASSWORD_FIELD)
...
def _read_or_seed_app_secret() -> dict[str, Any]:
    try:
    ...
    if missing:
        seeded = {
            ...
            DATABASE_PASSWORD_FIELD: settings.DATABASE_PASSWORD,
...
async def seed_demo_users() -> None:
    ...
    async with AsyncSessionLocal() as session:
        for email, credential, role in demo_users:
            if not credential:
                raise RuntimeError(f"Missing demo credential for {email}.")
            await seed_demo_user(session, email, credential, role)
        await session.commit()


def _require_secret_value(secret: dict[str, Any], field: str) -> str:
    value = secret.get(field)
    if not isinstance(value, str) or not value:
        raise RuntimeError(
            f"Vault secret {settings.VAULT_SECRET_BASE_PATH!r} is missing required field {field!r}."
        )
    return value
